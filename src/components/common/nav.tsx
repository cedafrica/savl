    ...
    return (
        ...
        >
            <div className="max-w-[1500px] mx-auto px-8 sm:px-20 py-5 flex items-center justify-between">
                        ...
                        <li key={item.label} className="relative group select-none">
                            ...
                            {item.type === "dropdown" ? (
                                <>
                                    ...
                                    <div className="
                                        absolute left-0 top-full mt-6 opacity-0 invisible
                                        group-hover:opacity-100 group-hover:visible
                                        group-hover:translate-y-0 translate-y-4
                                        transition-all duration-500
                                        bg-white text-black rounded-2xl shadow-xl px-10 py-8 w-[320px]
                                    ">
                ...
                <button
                    onClick={toggleNav}
                    className="sm:hidden z-[1000] w-10 h-10 flex flex-col justify-between relative"
                >
                    <span
                        className={`
                            h-[3px] w-full bg-white block transition-all duration-500 origin-left
                            ${open ? "rotate-45 translate-y-3" : ""}
                        `}
                    ></span>
                    <span
                        className={`
                            h-[3px] w-full bg-white block transition-all duration-500
                            ${open ? "opacity-0" : ""}
                        `}
                    ></span>
                    <span
            ...
            <div
                className={`
                    sm:hidden fixed inset-0 z-[900] bg-black/95 backdrop-blur-md
                        ...
                        <li key={item.label} className="flex flex-col">
                            ...
                            {item.type === "dropdown" ? (
                                <>
                                    ...
                                    <div
                                        className={`
                                            overflow-hidden transition-all duration-500 ml-2
                                            ${openApplication ? "mt-6 max-h-[1000px]" : "max-h-0 mt-0"}
                                        `}
                                    >
